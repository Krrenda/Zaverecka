...
def scrapovat_vysledky_voleb(base_url, vystupni_soubor):
    ...
    # Poèáteèní požadavek na získání hlavní stránky
    response = requests.get(base_url)
    if response.status_code != 200:
        print(f"Chyba: Nelze pøistoupit na {base_url}")
        return
    ...
    for odkaz_obec in odkazy_obce:
        ...
        if response_obec.status_code != 200:
            print(f"Chyba: Nelze pøistoupit na {obec_url}")
            continue
        ...
        try:
            ...
            strany = {strana.text.strip(): int(hlasy.text.strip().replace('\xa0', '').replace(' ', '').replace(',', '')) 
                      for strana, hlasy in zip(strany_data, hlasy_data[1::2])}  # každé druhé èíslo je hlas pro stranu
